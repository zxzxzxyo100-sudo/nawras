...
// ========== POST: إعادة تهيئة طابور المتابعة الدورية (حذف تعيينات active/no_answer ثم تعبئة من جديد) ==========
elseif ($action === 'bulk_clear_active_periodic') {
    $role = trim((string) ($input['user_role'] ?? ''));
    $username = trim((string) ($input['username'] ?? ''));
    $allManagers = !empty($input['all_active_managers']);
    if ($allManagers) {
        if ($role !== 'executive') {
            jsonResponse(['success' => false, 'error' => 'غير مصرّح — إعادة التهيئة لجميع المسؤولين للمدير التنفيذي فقط.'], 403);
        }
        $stmt = $pdo->query("SELECT username FROM users WHERE role = 'active_manager'");
        $users = $stmt ? $stmt->fetchAll(PDO::FETCH_COLUMN) : [];
        $users = array_values(array_filter(array_map('trim', $users)));
        if ($users === []) {
            jsonResponse(['success' => true, 'deleted_rows' => 0, 'filled_per_user' => [], 'message_ar' => 'لا يوجد مستخدمون بدور مسؤول المتاجر النشطة.']);
        }
    } else {
        if ($username === '') {
            jsonResponse(['success' => false, 'error' => 'username مطلوب'], 400);
        }
        if ($role !== 'active_manager' && $role !== 'executive') {
            jsonResponse(['success' => false, 'error' => 'غير مصرّح'], 403);
        }
        $users = [$username];
    }
    ...
    $deletedTotal = 0;
    $filledPerUser = [];
    $auditBy = $username !== '' ? $username : 'executive';
    foreach ($users as $u) {
        $u = trim((string) $u);
        if ($u === '') {
            continue;
        ...
        $n = $del->rowCount();
        $deletedTotal += $n;
        $filledPerUser[$u] = fill_slots_for_user($pdo, $u, $auditBy, null);
    }
    $detail = 'حذف تعيينات المتابعة الدورية (نشطة / لم يتم الرد) وإعادة التعبئة. صفوف محذوفة: ' . $deletedTotal;
    try {
        $pdo->prepare("
            INSERT INTO audit_logs (store_id, store_name, action_type, action_detail, performed_by, performed_role)
            VALUES (0, '', ?, ?, ?, ?)
        ")->execute([
            $allManagers ? 'طابور نشط — إعادة تهيئة (الكل)' : 'طابور نشط — إعادة تهيئة',
            $detail,
            $auditBy,
            $role,
        ]);
    ...
    jsonResponse([
        'success' => true,
        'deleted_rows' => $deletedTotal,
        'filled_per_user' => $filledPerUser,
        'message_ar' => $allManagers
            ? 'تمت إعادة تهيئة طوابير المتابعة الدورية لجميع مسؤولي المتاجر النشطة.'
            : 'تمت إعادة تهيئة طابور المتابعة الدورية. يمكنك الآن المتابعة من قائمة جديدة دون تعارض مع التعيينات السابقة.',
    ]);
}
