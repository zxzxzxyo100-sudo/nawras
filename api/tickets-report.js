...
exports.handler = async (event) => {
    const headers = { 'Access-Control-Allow-Origin': '*', 'Content-Type': 'application/json', 'Access-Control-Allow-Methods': 'GET, POST, OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type' };
    ...
    try {
        ...
        while (hasMore) {
            const body = { from: fromDate, to: to, date_type: 'created_at' };
            if (cursor) body.cursor = cursor;
            const data = await httpsPost('/tickets/report', body);
