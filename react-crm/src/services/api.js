...
export const getVipMerchantsMonthly = ({ months, threshold = 300, fromMonth, toMonth } = {}) => {
  const params = { threshold }
  if (fromMonth && toMonth) {
    params.from_month = fromMonth
    params.to_month = toMonth
  } else {
    params.months = months ?? 2
  }
  return http.get('/vip-merchants-monthly.php', { params }).then(r => r.data)
}
...
/**
 * متاجر سُجّلت ضمن فترة + منها شحنت + نسبة التحويل (من ذاكرة all-stores).
 * @param {{ detail?: boolean, from?: string, to?: string }} [opts]
 *   — من/إلى بصيغة YYYY-MM-DD (توقيت الرياض على الخادم). بدونهما: الشهر الحالي.
 *   — detail=true يعيد report_rows
 */
