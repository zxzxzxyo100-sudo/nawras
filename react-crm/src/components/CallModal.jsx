...
export default function CallModal({ store, callType = 'general', onClose, onSaved }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      // هل سيبلغ الهدف اليومي بعد هذه المكالمة؟
      const willHitGoal = (todayCalls + 1) >= DAILY_GOAL && todayCalls < DAILY_GOAL

      onCallSaved(pts)
      setCoinTrigger(Date.now())
      if (willHitGoal) setShowJackpot(true)

      onSaved?.()

      // أغلق Modal بعد لحظة لإتاحة مشاهدة الأنيميشن
